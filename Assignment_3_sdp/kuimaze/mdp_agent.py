    ...
    while True: 
        ...
        for state in all_states: 
            ...
            for action in problem.get_actions(state): 
                value = sum((probability*(state[2]+ discount_factor*state_reward_actions[next_state[0:2]][0])) for next_state, probability in problem.get_next_states_and_probs(state, action))
                if value > max_value: 
                    max_value = value 
                    max_action = action 
